...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.Dealer.coords, true) < 1.50 then
...
function OpenShop()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop', {
		...
		TriggerServerEvent('esx_legalfarming:sellStuff', data.current.name, data.current.value)
...
function OpenBuyLicenseMenu(licenseName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'license_shop', {
		...
		if data.current.value ~= 'no' then
			ESX.TriggerServerCallback('esx_legalfarming:buyLicense', function(boughtLicense)
